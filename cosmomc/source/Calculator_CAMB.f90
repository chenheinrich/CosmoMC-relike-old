  ...
  subroutine CAMBCalc_CMBToCAMB(this,CMB,P)
    ...
    use_basis = CosmoSettings%reion_use_basis
    nbasis = CosmoSettings%reion_nbasis
    if (use_basis .eq. .true.) then
      P%Reion%redshift = 0.0_dl
      P%Reion%delta_redshift = 0.0_dl
    else
      P%Reion%redshift = CMB%zre
      P%Reion%delta_redshift = CMB%zre_delta
    end if
    if (use_basis .eq. .true.) then
      call xe_basis%init(CosmoSettings%reion_basis_file, &
       CosmoSettings%reion_xe_fiducial,CosmoSettings%reion_sigma_smoothing) 
      P%Reion%nbasis = nbasis
      xe_basis%nbasis = nbasis
      P%Reion%mj(1:nbasis) = CMB%reion_mj(1:nbasis)
    end if
    P%Reion%AccBoost = CosmoSettings%reion_accuracy_boost 
    P%Reion%fraction = CosmoSettings%reion_fraction
    !VM CH ENDS
    w_lam = CMB%w
    wa_ppf = CMB%wa
    ALens = CMB%ALens
    ...
    if (CMB%omnuh2>0) then
      ...
      if (CMB%omnuh2>CMB%omnuh2_sterile) then
        neff_massive_standard = &
          CosmoSettings%num_massive_neutrinos*default_nnu/3
        ...
        P%Nu_mass_numbers(P%Nu_mass_eigenstates) = &
         CosmoSettings%num_massive_neutrinos
        P%Nu_mass_degeneracies(P%Nu_mass_eigenstates) = neff_massive_standard
        P%Nu_mass_fractions(P%Nu_mass_eigenstates) = &
         (CMB%omnuh2-CMB%omnuh2_sterile)/CMB%omnuh2
      else
        neff_massive_standard=0
      end if
      if (CMB%omnuh2_sterile>0) then
        if (CMB%nnu<default_nnu) then
          call MpiStop('nnu < 3.046 with massive sterile')
        end if
        ...
        P%Nu_mass_degeneracies(P%Nu_mass_eigenstates) = &
          max(1d-6,CMB%nnu - default_nnu)
        P%Nu_mass_fractions(P%Nu_mass_eigenstates) = &
          CMB%omnuh2_sterile/CMB%omnuh2
      end if
    end if
    P%YHe = CMB%YHe
#ifdef COSMOREC
  ...
  subroutine CAMBCalc_SetDerived(this,Theory)
    ...
    !CH BEGINS
    Theory%our_derived_tau = CP%our_derived_tau
    !CH ENDS
  ...
  subroutine CAMBCalc_GetNewTransferData(this, CMB,Info,Theory,error)
    ...
    !VM BEGINS ! to do: not sure if useful
    if(CosmoSettings%reion_use_basis .eq. .true.) then
      CMB%tau = CP%our_derived_tau
      write(*,*) 'CMB%tau = ', CMB%tau
    end if
    !VM ENDS
  ...
  subroutine CAMBCalc_GetNewPowerData(this, CMB, Info, Theory, error)
    ...
    class is (CAMBTransferCache)
      ...
      if(CosmoSettings%use_CMB) then
        !VM CH: WE NEED TO CHANGE THIS FUNCTION TO SETUP NEW TAU
  ...
  subroutine CAMBCalc_InitCAMB(this,CMB,error, DoReion)
    ...
    class(CMBParams), intent(inout) :: CMB
    ...
    write(*,*) 'GET OPTICAL DEPTH OMG NO NO NO NO'
