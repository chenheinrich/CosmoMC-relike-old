    ...
    integer :: numderived = 0, j, status

    if (ChainOutFile%unit==0) return

    !call Config%Parameterization%CalcDerivedParams(this%P, this%Theory, derived)
    allocate(derived(this%num_derived), STAT=status) 
    if(status .ne. 0) then
        write (*,*) 'Error (TCalculationAtParamPoint_WriteParams): memory allocation failed'
        stop
    endif
    derived(1:this%num_derived) = this%derived
    ...
    output_array(:num_params_used) = this%P(:num_params_used)
