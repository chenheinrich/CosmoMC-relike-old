    ...
    !VM CH WE MIGHT WANT TO HAVE SOME CLS AS DERIVED PARAMETERS
    integer, parameter :: max_derived_parameters = 500
    integer, parameter :: max_reion_nbasis = 100
    !VM CH ENDS
    ...
    Type TCosmoTheoryParams
      ...
      !VM BEGINS
      logical :: reion_use_basis
      integer :: reion_nbasis
      character(LEN=1024) :: reion_basis_file
      real(mcp) :: reion_accuracy_boost
      real(mcp) :: reion_sigma_smoothing 
      real(mcp) :: reion_xe_fiducial 
      real(mcp) :: reion_fraction
      !VM ENDS
    ...
    Type, extends(TTheoryParams) :: CMBParams
        ...
        real(mcp) reserved(5)
  			!VM BEGINS
  			real(mcp) reion_mj(max_reion_nbasis)
  			!VM ENDS
    end Type CMBParams
