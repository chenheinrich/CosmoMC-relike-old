    ...
    type CAMBparams
        ...
        real(dl) our_derived_tau !reionization tau
    end type CAMBparams
    ...
    !CH BEGINS: print constants for independent tau code (to delete)
    !write(*,*) printing constants for independent tau code:
	!write(*,'("sigma_T             	= ",e20.12)') sigma_thomson
	!write(*,'("8piG            		= ",e20.12)') kappa
	!write(*,'("m_H            		= ",e20.12)') m_H
	!write(*,'("mass_ratio_He_H      = ",e20.12)') mass_ratio_He_H
	!write(*,'("c            		= ",e20.12)') c
	!write(*,'("sigma_B            	= ",e20.12)') sigma_boltz
	!write(*,'("Mpc		           	= ",e20.12)') Mpc
	!write(*,'("tcmb		           	= ",e20.12)') CP%tcmb
	!write(*,*)
	!write(*,*) printing values for the current model:
	!write(*,'("YHe            		= ",e20.12)') CP%YHe
	!write(*,'("Nnow           		= ",e20.12)') Nnow
	!write(*,'("akthom             	= ",e20.12)') akthom
	!write(*,'("8piG*rho_g/c^2		= ",e20.12)') grhog
	!write(*,'("1.4952d-13*tcmb**4	= ",e20.12)') 1.4952d-13*CP%tcmb**4
	!write(*,*)
	!write(*,'("8piG*rho_nu1/c^2	= ",e20.12)') grhor
	!write(*,'("1.4952d-13*tcmb**4	= ",e20.12)') 3.3957d-14*CP%tcmb**4
	!write(*,*)
	!write(*,'("nu massless (want 2) = ",e20.12)') nu_massless_degeneracy
	!write(*,*)
	!write(*,'("3h0^2/c^2 = 8piG*rho_cri/c^2 	= ",e20.12)') grhom
	!write(*,'("grhom=3.3379d-11*h0*h0 			= ",e20.12)') 3.3379d-11*CP%h0*CP%h0
	!write(*,'("h0 								= ",e20.12)') CP%h0
	!CH ENDS
	
	!CH BEGINS: print cosmological parameters during iterations (to delete)
	!write(*,'("H0             		= ",f9.6)') CP%H0
    ...
    if (.not.call_again) then
        ...
        if (global_error_flag==0) then
            ...
            last_tau0=CP%tau0
            if (WantReion) then
              call Reionization_Init(CP%Reion,CP%ReionHist, &
...
500     if (L< lmax_extrap_highl) &
    ...
    private		
    !VM BEGINS
		!HIGHER NUMBER OF POINTS REDUCE THE DIFFERENCE BETWEEN TRAPEZOID AND
		!INVERSE TRAPEZOID RULE
    !integer,parameter :: nthermo=20000
		integer,parameter :: nthermo=30000
		!VM ENDS
    ...
    if (i < 1) then
        ...
        !VM BEGINS: COPY FROM MORTOSON
        !RPC
        ! Limit how small opacity can be to avoid divergence of 1/opacity
        if (abs(opacity) < 1.d-8) then 
          opacity = sign(1.d-8,opacity) ! 1e-8 with the sign of opacity
        end if
        !END RPC
        !VM ENDS
  ...
  subroutine inithermo(taumin,taumax)
    ...
    integer noutput
    !VM BEGINS
    integer status, ncount2
    real(dl), dimension(:), allocatable :: reion_a,recom_xe,recom_z
    !VM ENDS
    !CH BEGINS
    real(dl) z, xe_with_helium 
    logical :: write_grhonu = .false. 
    integer :: output_grhonu
    external output_grhonu
    integer :: done_output_grhonu = 0
    !CH ENDS
    external rombint
    ...
    sdotmu(1)=0
		...
		!VM BEGINS
		ncount2 = 0
		if(use_basis .eqv. .true.) then
	    !TASK: THREAD REIONIZATION_XE CALCULATION
      !FIRST: GET Z ARRAY
      !SECOND: CALCULATE REIONIZATION_XE
      !THIRD: RESET VARIABLES TO IC
	    do i=2,nthermo
		    ...
		    a=a0+2._dl*dtau/(1._dl/adot0+1._dl/adot)
				!VM: WE ARE NOT THREADING RECOMBINATION_XE BECAUSE WE DONT KNOW
        !IF THIS FUNCITON IS THREAD SAFE
        
        xe(i) = Recombination_xe(a)
        
        if (CP%Reion%Reionization .and. 1.0_dl/a-1.0_dl<Reionization_maxz) then
		      if(ncount2 == 0) then
		      	ncount2=i-1
						allocate(recom_z(nthermo-ncount2+1),recom_xe(nthermo-ncount2+1),&
              STAT=status)
						if(status .ne. 0) then
							write (*,*) 'Error (reionization): memory allocation failed'
							stop
						endif
            ! THE FIRST POINT IS THE LAST ONE BEFORE THIS IF STATEMENT WAS TRUE
						!recom_z(1) = 1.0_dl/a0-1
						!recom_xe(1) =	xe(i-1)
						recom_z(nthermo-ncount2+1) = 1.0_dl/a0-1
						recom_xe(nthermo-ncount2+1) =	xe(i-1)
		      end if
          !VM WE NEED THAT WHEN CALCULATING REIONIZATION (BECAUSE OF SMOOTHING)
					!recom_z(i-ncount2+1) = 1.0_dl/a-1.0_dl
					!recom_xe(i-ncount2+1) = xe(i)  
					recom_z(nthermo-i+1) = 1.0_dl/a-1.0_dl
					recom_xe(nthermo-i+1) = xe(i) 
					if(tau > CP%ReionHist%tau_start) then
						if(ncount == 0) then
					    ncount=i-1
							allocate(reion_a(nthermo-ncount+1),STAT=status)
							if(status .ne. 0) then
							  write (*,*) 'Error (reionization): memory allocation failed'
								stop
							endif	
      ...
      if (CP%Reion%Reionization .eqv. .true.) then
        if(size(reion_a) < 1) then
          call mpistop("Error IniThermo: Bad Array reion_a")
        end if
        !$OMP PARALLEL DO DEFAULT(SHARED),SCHEDULE(STATIC)
  	    do i=ncount,nthermo
          ! WE ONLY NEED TO REPLACE i > NCOUNT. i < NCOUNT IS JUST RECOMBINATION
  		  	xe(i)=Reionization_xe2(reion_a(i-ncount+1),recom_z,recom_xe)
  	    end do
  	    !$OMP END PARALLEL DO 
      end if
	    !VM RESET CODE TO IC STATE
	    ncount = 0
	    !  Initial conditions: assume radiation-dominated universe.
	    tau01=tauminn
	    adot0=adotrad
    ...
    do i=2,nthermo
      ...
      !VM BEGINS : COPY FROM MORTOSON CODE
      !fe=(1._dl-CP%yhe)*xe(i-1)/(1._dl-0.75d0*CP%yhe+(1._dl-CP%yhe)*xe(i-1))
      !RPC
      ! Keep xe between 0 and (1-Yp/2)/(1-Yp) (fully ionized H and He)
      ! for the definition of fe to avoid diverging quantities.
      if (xe(i-1)<=((1._dl-CP%yhe/2.d0)/(1._dl-CP%yhe)).and.xe(i-1)>=0.) then
        fe=(1._dl-CP%yhe)*xe(i-1)/(1._dl-0.75d0*CP%yhe+(1._dl-CP%yhe)*xe(i-1))
      else if (xe(i-1)<0.) then
        fe=1.d-4
      else
        fe=(1._dl-CP%yhe)*((1._dl-CP%yhe/2.d0)/(1._dl-CP%yhe))/(1._dl-&
          0.75d0*CP%yhe+(1._dl-CP%yhe)*((1._dl-CP%yhe/2.d0)/(1._dl-CP%yhe)))
      end if
      !END RPC
      !VM ENDS   
      ...
      if (CP%Reion%Reionization .and. tau > CP%ReionHist%tau_start) then
        ...
        if (CP%AccurateReionization .and. CP%DerivedParameters) then
          !CH BEGINS !TODO (need testing not taking out recomb.)
          dotmu(i)=(Recombination_xe(a) - xe(i))*akthom/a2
          !dotmu(i)= -xe(i)*akthom/a2
          !CH ENDS
		  !VM BEGINS : COPY FROM MORTOSON CODE
          !CH don't need those changes anymore because we are switching
          ! to trapzezoid rule
          !if (last_dotmu /=0) then
          !if (last_dotmu /=0 .and. dotmu(i) /= 0 .and. &
          	!abs(last_dotmu+dotmu(i))>abs(last_dotmu-dotmu(i))) then
					!VM ENDS
 					!VM BEGINS
 					!VM: REPLACE INVERSE TRAPEZOID TO AVOID PROBLEMS WHEN XE = 0
         	!actual_opt_depth=actual_opt_depth-&
 					  !	2._dl*dtau/(1._dl/dotmu(i)+1._dl/last_dotmu)	
 		  		  		  
          actual_opt_depth=actual_opt_depth-0.5_dl*dtau*(dotmu(i)+last_dotmu)	
          ! end if
          !VM ENDS 
          last_dotmu = dotmu(i)
        end if
      ...
      dotmu(i)=xe(i)*akthom/a2
      !VM COPIED FROM MORTOSON CODE
      !if (tight_tau==0 .and. 1/(tau*dotmu(i)) > 0.005) tight_tau = tau !0.005
       !Tight coupling switch time when k/opacity is smaller than     
       !1/(tau*opacity)
      if (tight_tau==0_dl .and. 1.0_dl/(tau*abs(dotmu(i)))>0.005) then
        tight_tau=tau !0.005
      end if
      !VM END
      if (tau < 0.001) then
        ...
        !VM BEGINS
  			...
  			sdotmu(i) = sdotmu(i-1)+0.5_dl*dtau*(dotmu(i)+dotmu(i-1))
        !VM ENDS
      end if
      a0=a
      tau01=tau
    ...
    !VM BEGINS
    CP%our_derived_tau = actual_opt_depth
    !VM ENDS
      
    !VM BEGINS
    if(use_basis .eqv. .true. ) then
			if(allocated(reion_a)) then
	  		deallocate(reion_a,STAT=status)
	  		if(status .ne. 0) then
	  			write (*,*) 'Error (reionization): memory allocation failed'
	  			stop
	  		endif	
	    end if
	    if(allocated(recom_z)) then
	  		deallocate(recom_z,STAT=status)
	  		if(status .ne. 0) then
	  			write (*,*) 'Error (reionization): memory allocation failed'
	  			stop
	  		endif	
	    end if
	    if(allocated(recom_xe)) then
	  		deallocate(recom_xe,STAT=status)
	  		if(status .ne. 0) then
	  			write (*,*) 'Error (reionization): memory allocation failed'
	  			stop
	  		endif	
    ...
    if (CP%AccurateReionization .and. FeedbackLevel > 0 .and. CP%DerivedParameters) then
        write(*,'("Reion opt depth      = ",f7.6)') actual_opt_depth
    end if
    ...
    if (dowinlens) call splder(winlens,dwinlens,nthermo,spline_data)
		...
		!VM BEGINS
		!do j1=2,nthermo
			!if(1.0_dl/scaleFactor(j1)-1 < 100) then 
			  !write(*,*) 1.0_dl/scaleFactor(j1)-1, xe(j1)
			  !write(*,*) 1.0_dl/scaleFactor(j1)-1, dotmu(j1), ddotmu(j1)
			  !write(*,*) 1.0_dl/scaleFactor(j1)-1, dddotmu(j1), ddddotmu(j1)
        !end if
		!end do
		!VM ENDS
    ...
    !CH BEGINS: output_grhonu defined in equations_ppf.f90
    if (write_grhonu .eqv. .true.) then
    	done_output_grhonu = output_grhonu() ! argument set to true to write grho total
    end if
    !CH ENDS
