...
1   if (khi-klo.gt.1) then
    ...
    grhok = 0.000000000000000E+000
    grhoc = 3.995382819431314E-008
    grhob = 7.424529124136492E-009
    grhog = 8.260148100755502E-012
    w_lam = -1.00000000000000     
    grhornomass = 3.809408986356058E-012
    grhov = 1.035798870387910E-007

    CP%Num_Nu_massive = 1
    CP%nu_mass_eigenstates = 1

    nu_masses(1) = 353.712881263422     
    grhormass(1) = 1.903307442173650E-012
	...
	!CH BEGINS
	function output_grhonu()
    !use constants
    ! write massive neutrino part of 8*pi*G*rho*a**4 between z = 3 and 40.
    use precision
    use ModelParams
    use MassiveNu
    use LambdaGeneral
	...
	output_grhonu = 0
		...
		write(*,*) "Writing to grhonu.dat from equations_ppf.f90"
		open(unit = 61, file = "grhonu.dat")
		! write(61,*) 'z,  effective 8*pi*G*rho_numass*a**4'
		! for calculating H(z) to get tau independently
		do j2=0,1000
			logz = -13.0 + j2*(4.0-(-13.0))/1000.0
			z = exp(logz)
			a = 1.d0/(1.d0+z)
			a2 = a**2
			!  8*pi*G*rho*a**4.
			...
			if (CP%Num_Nu_massive /= 0) then
				...
				do nu_i = 1, CP%nu_mass_eigenstates
					call Nu_rho(a*nu_masses(nu_i),rhonu)
					grhoa2=grhoa2+rhonu*grhormass(nu_i)
					!write(*,*) 'nu_i = ', nu_i, ' rhonu = ', rhonu, ' grhormass(nu_i)', grhormass(nu_i)
					write(61,*) z, rhonu*grhormass(nu_i)
				end do
			end if
		end do 
		close(61)
		write(*,*) "Done writing to grhonu.dat"
		write (*,*) 
		output_grhonu = 1  
	
    end function output_grhonu
	!CH ENDS
